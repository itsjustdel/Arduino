...
void Joystick_::setThrottle1(int16_t value)
{
	_throttle1 = value;
	if (_autoSendState) sendState();
}
...
void Joystick_::sendState()
{
	...
	//chucking in throttle1 here, may break values underneath
	index += buildAndSetSimulationValue(_includeSimulatorFlags & JOYSTICK_INCLUDE_THROTTLE, _throttle1, _throttleMinimum, _throttleMaximum, &(data[index]));
	//...
	//ok under here?
